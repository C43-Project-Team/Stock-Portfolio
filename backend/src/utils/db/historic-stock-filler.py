...
def fetch_data(ticker, start_date='2018-02-08', end_date='2024-07-11'):
    ...
    try:
        ...
        df['Code'] = ticker
        df = df[['Code', 'Open', 'Close', 'Low', 'High', 'Volume']]  # Select necessary columns
        df = df.round({'Open': 2, 'Close': 2, 'Low': 2, 'High': 2})  # Round to 2 decimal places
        df.index = df.index.date  # Format date to yyyy-mm-dd
        # df.reset_index(inplace=True)  # Reset index to get 'Date' as a column
        df.rename(columns={'Date': 'Timestamp'}, inplace=True)  # Rename columns
        return df
...
if __name__ == "__main__":
    ...
    all_data = pd.concat([fetch_data(ticker) for ticker in tickers])
